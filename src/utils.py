...
def draw_kinematics(ema_data, ema_data_hat, mode, title, **args):
    ...
    for i in range(6):
        ...
        for j in range(2):
            ...
            ax.plot(x, _data,c=colors[i], label='ori', linewidth=6)
            if mode == 'kinematics':
...
#             if j == 0:
#                 ax.set_ylabel(labels[i]+' x',rotation=0, fontsize=60, labelpad=10)
#             else:
#                 ax.set_ylabel(labels[i]+' y',rotation=0,fontsize=60, labelpad=10)
...
def vis_gestures(model, **args):
    ...
    gestures = model.gesture_weight.unsqueeze(1) * 1
