...
def trainer_pr(model, optimizer, lr_scheduler, ema_dataset_train, ema_dataset_test, **args):
    ...
    for e in range(args['num_epochs']):
        ...
        if (e+1) % args['eval_epoch'] == 0:
            ####start evaluation
            print("Eval! Currently there is no eval")
...
if __name__ == "__main__":
    ...
    if args.pr_h or args.pr_mel or args.pr_ema:
        trainer_pr(model, optimizer, lr_scheduler, ema_dataloader_train, ema_dataloader_test,  **vars(args))
    elif args.vq_only:
