...
def kmeans_ema_art(spk_id_setting='mngu0'):
...
#         super_ema_data_huge = super_ema_data_huge[:100000]
        ...
        cluster_ids, cluster_centers = kmeans(X=super_ema_data_huge, num_clusters=2, distance='euclidean', device=device)

        print("kmeans finished!!")
        print("shape of centers of kmeans is", cluster_centers.shape)
        print("shape of ids of kmeans is", cluster_ids.shape)
        
        kmeans_centers.append(cluster_centers.detach().numpy())
        
    kmeans_centers = np.concatenate(kmeans_centers, axis=0)
    np.save(f"kmeans_centers_art_2.npy", kmeans_centers)
